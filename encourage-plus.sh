...
if [ $LEVEL -eq 0 ]; then
	...
	echo Mommy on set commands
	# set commands get upgraded
	if [[ ! -v PRE_MOMMY_PS1 ]]; then
		PRE_MOMMY_PS1=$PS1
		PS1="<3 $PS1"
	fi
	alias curl="mommy-plus.py curl --fail-with-body"
	alias gcc="mommy-plus.py gcc"
	alias cbmc="mommy-plus.py cbmc"
	if [ ! -z ${PRE_MOMMY_PROMPT_COMMAND+x} ]; then
		PROMPT_COMMAND=$PRE_MOMMY_PROMPT_COMMAND
	fi
	unset PRE_MOMMY_PROMPT_COMMAND
elif [ $LEVEL -eq 2 ]; then
	echo Mommy only encourages
	...
	if [ -z ${PRE_MOMMY_PROMPT_COMMAND+x} ]; then
		...
		PROMPT_COMMAND="mommy-plus.py -s \$?; $PROMPT_COMMAND"
	fi
else
	echo Mommy everywhere
